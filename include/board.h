...
/* ─────────────────────────────────────────────────────────
 * LOG FILE HEADER — Written at offset 0 of inspection_log.bin
 * Updated via lseek() after every board completes.
 * NOTE: Size is platform-dependent (time_t is 8 bytes on 64-bit).
 *       We use sizeof(log_header_t) consistently — no hardcoded size.
 * ───────────────────────────────────────────────────────── */
typedef struct {
    uint32_t    magic;                  /* 0xCB123456 — file integrity check */
